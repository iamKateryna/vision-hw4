    ...
    for (int i = 0; i < im.w; ++i) {
        for (int j = 0; j < im.h; ++j) {
            for (int k = 0; k < im.c; ++k) {
                float starting_point = get_pixel_check_bounds(im, i, j, k); //starting point (pixel value of orig image)
                float top_right = get_pixel_check_bounds(integ, i, j - 1, k); //top right
                float bottom_left = get_pixel_check_bounds(integ, i - 1, j, k); //bottom left
                float top_left = get_pixel_check_bounds(integ, i - 1, j - 1, k); //top left

                float pixel_value = starting_point + bottom_left + top_right - top_left;
...
image box_filter_image(image im, int s)
{
        ...
        for(i=0-offset; i<im.w-offset; i++) {
            for (j = 0 - offset; j < im.h - offset; j++) {
                for (k = 0; k < im.c; k++) {
                    float D = get_pixel(integ, i, j, k);
                    float B = get_pixel(integ, i + s, j, k);
                    float C = get_pixel(integ, i, j + s, k);
                    float A = get_pixel(integ, i + s, j + s, k);
                    float pixel_value = D - B - C + A;
                    set_pixel(S, S.w, S.h, S.c, pixel_value);
                }
            }
        }
        free_image(integ);
...
image time_structure_matrix(image im, image prev, int s)
{
    ...
    image Ix = convolve_image(im, gx_filter, 1);
    image Iy = convolve_image(im, gy_filter, 1);
    image S = make_image(im.w, im.h, 5);
    for(int i=0; i<im.w; i++)
    {
        for(int j=0; j<im.h; j++)
        {
            float Ix_i = get_pixel(Ix, i, j, 0);
            float Iy_i = get_pixel(Iy, i, j, 0);
            float It_i = get_pixel(prev, i, j, 0) - get_pixel(im, i, j, 0);
            set_pixel(S, i, j, 0, Ix_i*Ix_i);
            set_pixel(S, i, j, 1, Iy_i*Iy_i);
            set_pixel(S, i, j, 2, Ix_i*Iy_i);
            set_pixel(S, i, j, 3, Ix_i*It_i);
            set_pixel(S, i, j, 4, Iy_i*It_i);
        }
    }
    if(converted){
...
image velocity_image(image S, int stride)
{
    ...
    for(j = (stride-1)/2; j < S.h; j += stride){
        for(i = (stride-1)/2; i < S.w; i += stride){
            ...
            float detS = M.data[0][0]*M.data[1][1] - M.data[1][0]*M.data[0][1];
            float vx = 0;
            float vy = 0;
            if(detS!=0)
            {
                ...
                matrix vel_matrix = matrix_mult_matrix(M_invert,N);
                vx = vel_matrix.data[0][0];
                vy = vel_matrix.data[1][0];
            }
